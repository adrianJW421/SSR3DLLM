...
if __name__ == '__main__':
    ...
    # Resolve mapping files relative to the referit3d package, not the current working directory.
    referit3d_root = Path(__file__).resolve().parents[1]  # .../referit3d
    idx_to_semantic_class_file = referit3d_root / 'data' / 'mappings' / 'scannet_idx_to_semantic_class.json'
    instance_class_to_semantic_class_file = (
        referit3d_root / 'data' / 'mappings' / 'scannet_instance_class_to_semantic_class.json'
    )
    axis_alignment_info_file = referit3d_root / 'data' / 'scannet' / 'scans_axis_alignment_matrices.json'

    scannet = ScannetDataset(args.top_scan_dir,
                             str(idx_to_semantic_class_file),
                             str(instance_class_to_semantic_class_file),
                             str(axis_alignment_info_file))
    ...
    if args.n_processes == -1:
        ...
        n_processes = min(int(args.n_processes), n_items)

    pool = mp.Pool(n_processes)
    chunks = max(1, int(n_items / n_processes))
