...
# coding: utf-8
"""
CUDA_VISIBLE_DEVICES=0 python scripts/train_referit3d_pre.py \
...
# script directly, without relying on the global Python environment or CWD.
        ...
        for phase in ['train', 'test']:
            ...
            if args.obj_cls_alpha > 0:
                if getattr(args, "use_scannet200_obj_cls", False):
                    info += ', ScanNet200-Obj-Acc: {:.4f}'.format(meters[phase + '_object_cls_acc'])
                    if (phase + '_scannet_valid_frac') in meters:
                        info += ', ScanNet200-ValidFrac: {:.3f}'.format(meters[phase + '_scannet_valid_frac'])
                else:
...
if __name__ == '__main__':
    ...
    # Keep on CPU here; each DataParallel replica will move it to its own device
    # inside the model forward (avoids cuda:0 vs cuda:1 mismatches).
    ...
    if gpu_num > 1:
        ...
        if getattr(model.module, "object_encoder", None) is not None:
            param_list.append({'params':model.module.object_encoder.parameters(), 'lr':args.init_lr})
        else:
            if getattr(model.module, "mask3d_proj_in", None) is not None:
                param_list.append({'params':model.module.mask3d_proj_in.parameters(), 'lr': args.init_lr})
            if getattr(model.module, "mask3d_adapter", None) is not None:
                param_list.append({'params':model.module.mask3d_adapter.parameters(), 'lr': args.init_lr})
        param_list.extend([
        ...
        if getattr(args, "use_scannet200_obj_cls", False) and getattr(model.module, "scannet_obj_clf", None) is not None:
            param_list.append({'params':model.module.scannet_obj_clf.parameters(), 'lr': args.init_lr})
    else:
        param_list=[
            {'params':model.language_encoder.parameters(),'lr':args.init_lr*0.1},
        ]
        if getattr(model, "object_encoder", None) is not None:
            param_list.append({'params':model.object_encoder.parameters(), 'lr':args.init_lr})
        else:
            if getattr(model, "mask3d_proj_in", None) is not None:
                param_list.append({'params':model.mask3d_proj_in.parameters(), 'lr': args.init_lr})
            if getattr(model, "mask3d_adapter", None) is not None:
                param_list.append({'params':model.mask3d_adapter.parameters(), 'lr': args.init_lr})
        param_list.extend([
        ...
        if getattr(args, "use_scannet200_obj_cls", False) and getattr(model, "scannet_obj_clf", None) is not None:
            param_list.append({'params':model.scannet_obj_clf.parameters(), 'lr': args.init_lr})
