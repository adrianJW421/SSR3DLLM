...
class batch_lang_data:
    def __init__(self,
        ...
        # Per-raw-text original strings, aligned with raw_lang_types / raw_target_gt_ids.
        # This avoids fragile reconstruction from concatenated strings in trainer code.
        ...
        # This is used to lookup teacher-forced referential_order from grounding_steps JSON.
        self.raw_target_gt_ids = raw_target_gt_ids
    ...
    def print(self):
        ...
        if self.raw_texts is not None:
            print(f'- raw_texts: [{len(self.raw_texts)}, ?] = shape of {[len(i) if i is not None else 0 for i in self.raw_texts]}')
        print(
        ...
        if self.raw_target_gt_ids is not None:
            print(
                f'- raw_target_gt_ids: [{len(self.raw_target_gt_ids)}, ?] = shape of {[len(i) if i is not None else 0 for i in self.raw_target_gt_ids]}'
            )
    ...
    if "labels" in input_dict:
        ...
        if len(list_labels[0].shape) == 1:
            ...
            if mode == "test":
                ...
                # Keep the original (per-raw-text) strings to avoid fragile splitting logic later.
                raw_texts = [getattr(lang_bid, "texts", None) for lang_bid in extra_lang]
                ...
                raw_target_gt_ids = [None] * len(extra_lang)
                ...
                if len(batch_concat_texts) != 0:
                    ...
                    for bid in range(len(extra_lang)):
                        ...
                        for lang_id, (gt_inst_ids, positives) in enumerate(zip(
                            ...
                            if (
                                hasattr(extra_lang[bid], "concat_target_gt_ids")
                                and isinstance(extra_lang[bid].concat_target_gt_ids, (list, tuple))
                                and lang_id < len(extra_lang[bid].concat_target_gt_ids)
                            ):
                                raw_target_gt_ids_bid.append(extra_lang[bid].concat_target_gt_ids[lang_id])
                            else:
                                raw_target_gt_ids_bid.append(None)
                        ...
                        raw_target_gt_ids[bid] = raw_target_gt_ids_bid
                    ...
                    extra_lang = batch_lang_data(
                        batch_concat_texts, tokenized, batch_num_concat_texts, raw_texts, raw_phrases, flatten_lang_token_inst_id_pairs, raw_lang_types,
                        raw_target_gt_ids,
...
def get_instance_masks(
    ...
    for batch_id in range(len(list_labels)):
        ...
        masks = []
        segment_masks = []
        # Keep original ScanNet instance ids for each mask row (same order as `masks`).
        # This is useful for downstream evaluation that still refers to original instance ids.
        orig_instance_ids = []
        ...
        for instance_id in instance_ids:
            ...
            orig_instance_ids.append(int(instance_id))
            ...
            target.append(
                {
                    ...
                    "orig_instance_ids": orig_instance_ids,
