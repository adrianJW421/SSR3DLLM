...
class DotProdWithIdx_v3(Function):
    ...
    def forward(
        ...
        # ctx.T = T
        ctx.save_for_backward(
            q, index_q_offsets, k, index_k, table_q, table_k, rel_idx
        )
        return output
...
def queryandgroup(
    ...
    if idx is None:
        idx, _ = knnquery(
            nsample, xyz, new_xyz, offset, new_offset
        )  # (m, nsample)
    ...
    if use_xyz:
        if return_indx:
            return (
                torch.cat((grouped_xyz, grouped_feat), -1),
                idx,
            )  # (m, nsample, 3+c)
        else:
...
def interpolation_v2(xyz, new_xyz, feat, offset, new_offset, k=3):
    ...
    dist = torch.sqrt(
        ((new_xyz.unsqueeze(1) - xyz[idx.long()]) ** 2).sum(-1) + 1e-8
    )
...
class Interpolation(Function):
    ...
    def forward(ctx, xyz, new_xyz, input, offset, new_offset, k=3):
        ...
        idx, dist = knnquery(
            k, xyz, new_xyz, offset, new_offset
        )  # (n, k), (n, k)
