...
class Mask3D(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if self.clip_classifier:
            self.bg_class = nn.Parameter(torch.randn(1, hidden_dim), requires_grad=True)
            torch.nn.init.xavier_normal_(self.bg_class)
            self.clip_adapter = nn.Linear(512, hidden_dim)
            self.fg_classes_embed = torch.load('./pretrained/label_clip_features.pth')
            self.query_class_embed_head = nn.Linear(hidden_dim, hidden_dim)
        else:
            assert self.softmax_mode
    ...
    def forward(
        ...
        predictions_class = []
        predictions_mask = []
        predictions_query_instance_mask = []
    ...
    def mask_module(
        ...
        num_queries = query_feat.shape[1]
        query_feat = self.decoder_norm(query_feat)
        
        mask_embed = self.mask_embed_head(query_feat)
        if self.clip_classifier:
            if self.softmax_mode:
                class_embed_head = torch.cat([self.clip_adapter(self.fg_classes_embed.cuda()), self.bg_class.cuda()])
            else:
                class_embed_head = self.clip_adapter(self.fg_classes_embed.cuda())
            class_embed_head = class_embed_head[None]
            query_class_embed = self.query_class_embed_head(query_feat)

            outputs_class = torch.einsum('ijk,ibk->ijb', 
                query_class_embed / (query_class_embed.norm(dim=-1, keepdim=True) + 1e-8),
                class_embed_head / (class_embed_head.norm(dim=-1, keepdim=True) + 1e-8),
                ) / self.temperature

            normalized_query_embed = query_class_embed / (query_class_embed.norm(dim=-1, keepdim=True) + 1e-8)
        else:
