...
class grounded_3d_llm_data:
    def __init__(self,
        ...
        # Optional pointer back to the originating lang_info_data object.
        # This allows downstream modules (e.g., SSR3DLLM geometry head) to
        # attach LLM-derived context vectors to the language sample without
        # threading extra structures through the trainer.
        self.lang_info: Optional[Any] = lang_info
        # Optional LLM text context used as initial state for referential
        # order decoders. This is typically populated from the LLM's last
        # hidden layer and projected into instance feature space.
        self.llm_text_init: Optional[torch.Tensor] = None
    ...
    def build_input_from_segments(tokenizer,  # from LLama3dForCausalLM.llama_tokenizer
        ...
        if use_input_referent and not disable_grounding:
        ...
        last_end = 0
        if (not disable_grounding) and answer_grounding_token_pos and answer_grounding_token_pos[0]:
        ...
        if disable_grounding:
            instance['ref_token_mask'] = torch.zeros(
                instance["lm_labels"].shape[0], dtype=bool)
        else:
