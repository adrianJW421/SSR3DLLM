...
import copy
from baseline.dataset.dataset_code.utils import read_axis_align_matrix
...
def eval_seg_model(bid,
    ...
    # Prefer the per-raw-text strings provided by the collate fn. This is robust to
    # punctuation (e.g., "." inside an utterance) and avoids fragile splitting.
    lang_texts = []
    ...
    if (
        ...
        try:
            lang_texts = [str(x) for x in raw_texts_all[bid]]
        except Exception:
            lang_texts = []

    # Backward-compatible fallback: split concatenated text blocks.
    if not lang_texts:
        try:
            n_concat_all = getattr(extra_lang, "batch_num_concat_texts", None) or []
            n_concat = int(n_concat_all[bid]) if bid < len(n_concat_all) else 0
            start = int(sum(int(x) for x in (n_concat_all[:bid] if bid > 0 else [])))
            joined = "".join(extra_lang.batch_concat_texts[start : start + n_concat])
            lang_texts = joined.split(". ")[:-1]  # remove last empty
        except Exception:
            lang_texts = []

    # Best-effort alignment: avoid crashing evaluation on rare separator/tokenization edge cases.
    n_texts = len(lang_texts)
    n_pairs = len(flatten_lang_token_inst_id_pair) if flatten_lang_token_inst_id_pair is not None else 0
    n_types = len(raw_lang_type) if raw_lang_type is not None else 0
    n = min(n_texts, n_pairs, n_types)
    if not (n_texts == n_pairs == n_types):
        try:
            if str(getattr(config, "general", None) or "").strip() and getattr(config.general, "train_mode", False) is not None:
                pass
        except Exception:
            pass
        try:
            print(
                f"[eval_seg_model][WARN] length mismatch; truncating to min length: "
                f"texts={n_texts} pairs={n_pairs} types={n_types} -> {n}",
                flush=True,
            )
        except Exception:
            pass
    if n <= 0:
        ...
        return empty, [], [], [], []
    lang_texts = lang_texts[:n]
    flatten_lang_token_inst_id_pair = flatten_lang_token_inst_id_pair[:n]
    raw_lang_type = raw_lang_type[:n]
    ...
    if isinstance(aligned_scene_coordinates, torch.Tensor):
        aligned_scene_coordinates = aligned_scene_coordinates.detach().cpu().numpy()
    else:
        aligned_scene_coordinates = np.asarray(aligned_scene_coordinates)

    def _ensure_cpu_float_tensor(tensor_like):
        if tensor_like is None:
            return None
        if isinstance(tensor_like, torch.Tensor):
            return tensor_like.detach().to(torch.float32).cpu()
        return torch.as_tensor(tensor_like, dtype=torch.float32)

    raw_masks = _ensure_cpu_float_tensor(raw_masks)
    raw_heatmap = _ensure_cpu_float_tensor(raw_heatmap)
    num_points = aligned_scene_coordinates.shape[0]
    num_queries = int(pred_lang_logits.shape[0]) if hasattr(pred_lang_logits, "shape") else 0
    if raw_masks is None or raw_heatmap is None:
        raw_masks = torch.zeros((num_points, num_queries), dtype=torch.float32)
        raw_heatmap = torch.zeros((num_points, num_queries), dtype=torch.float32)

    target_masks = target_full_res[bid].get('masks')
    if target_masks is None:
        raise RuntimeError(f"Missing ground-truth instance masks for bid {bid}.")
    if isinstance(target_masks, torch.Tensor):
        target_masks_np = target_masks.detach().cpu().numpy()
    else:
        target_masks_np = np.asarray(target_masks)
    
    pred_lang_logits = pred_lang_logits

    # `pred_lang_logits` has shape [num_queries, num_phrases_used_by_model].
    # However, the number of *detection* phrases prepended in the dataset can vary
    # (e.g., when classes are filtered). Using a fixed `num_classes-1` offset can
    # therefore produce rare out-of-bounds errors at evaluation time.
    n_lang_logits = int(pred_lang_logits.shape[1]) if hasattr(pred_lang_logits, "shape") else 0
    if n_lang_logits <= 0:
        empty = (
            np.zeros((0,), dtype=float),
            np.zeros((0,), dtype=str),
    ...
    legacy_start_token = (config.model.num_classes - 1) if not config.data.sample_class_labels or not training else 0

    # Infer detection phrase prefix length from the actual batch metadata.
    det_phrase_max = -1
    try:
        for ti, t in enumerate(raw_lang_type):
            if not isinstance(t, str) or (not t.startswith("detection")):
                continue
            for token_bid, _gt_inst_id in flatten_lang_token_inst_id_pair[ti]:
                try:
                    det_phrase_max = max(det_phrase_max, int(token_bid))
                except Exception:
                    continue
    except Exception:
        det_phrase_max = -1
    det_offset = int(det_phrase_max + 1) if det_phrase_max >= 0 else int(legacy_start_token)

    # One-time debug print (helps diagnose protocol mismatches on clusters).
    try:
        if not hasattr(eval_seg_model, "_warned_det_offset"):
            eval_seg_model._warned_det_offset = True  # type: ignore[attr-defined]
            print(
                f"[eval_seg_model] det_offset={det_offset} legacy_start={legacy_start_token} n_lang_logits={n_lang_logits}",
                flush=True,
            )
    except Exception:
        pass
    ...
    # Iterate over all texts but skip detection items by type (more robust than fixed offsets).
    for i in range(0, len(lang_texts)):
        eval_type = raw_lang_type[i]
        if isinstance(eval_type, str) and eval_type.startswith("detection"):
            continue
        ...
        for now_token_id in all_unique_token_ids:
            ...
            # GT phrases box
            gt_extra_masks.append( target_masks_np[ inst_ids ].sum(axis=0)>0 ) # concate all gt insts
            try:
                gt_extra_query_texts.append((lang_phrase[int(now_token_id)], lang_texts[i]))
            except Exception:
                # Skip malformed phrase indices (rare tokenization edge cases).
                continue

            # Pred phrases
            # Primary mapping: remove the inferred detection prefix from phrase ids.
            # Fallbacks: legacy fixed prefix, then direct indexing.
            candidates = []
            try:
                candidates.append(int(now_token_id) - int(det_offset))
            except Exception:
                pass
            try:
                candidates.append(int(now_token_id) - int(legacy_start_token))
            except Exception:
                pass
            try:
                candidates.append(int(now_token_id))
            except Exception:
                pass

            pos_token_ids = None
            for c in candidates:
                if 0 <= int(c) < int(n_lang_logits):
                    pos_token_ids = int(c)
                    break
            if pos_token_ids is None:
                try:
                    if not hasattr(eval_seg_model, "_warned_oob_phrase"):
                        eval_seg_model._warned_oob_phrase = True  # type: ignore[attr-defined]
                        print(
                            f"[eval_seg_model][WARN] phrase_id out of range; "
                            f"phrase_id={now_token_id} det_offset={det_offset} legacy_start={legacy_start_token} "
                            f"n_lang_logits={n_lang_logits} eval_type={eval_type}. Skipping.",
                            flush=True,
                        )
                except Exception:
                    pass
                ...
                if not inst_ids.shape[0]: # no gt
                    ...
                    try:
                        ...
                        for gt_mask_id in inst_ids:
                            m3d_gt_mask = target_masks_np[ gt_mask_id ]
                        ...
                        raise RuntimeError(
                            f"Failed to compute language-guided metrics (bid={bid}, token_id={now_token_id}, eval_type={eval_type}): {e}"
                        ) from e
            ...
            for qi in np.where(pred_query_ids)[0]:
                ...
                coord = aligned_scene_coordinates[pred_mask.numpy().astype(bool)].mean(axis=0)
