...
try:
    # Optional: used for Mask3D detection / instance metrics.
    # Keep import-time robust so baseline can be imported without vendored `benchmark/`.
    from benchmark.evaluate_semantic_instance import evaluate as mask3d_det_evaluation  # type: ignore
except Exception:  # pragma: no cover
    mask3d_det_evaluation = None  # type: ignore
import glob
import pickle
...
def _resolve_scannet200_instance_gt_path() -> str:
    """
    Resolve ScanNet200 instance GT directory for detection evaluation.

    The previous hard-coded path (`./data/processed/scannet200/...`) is not portable
    across machines/cluster layouts and can crash evaluation if GT files are missing.

    Priority:
      1) SSR3DLLM_SCANNET200_INSTANCE_GT_PATH (absolute path to instance_gt/<split>)
      2) config.SCANNET_PROCESSED_ROOT/instance_gt/<split> (split defaults to "validation")
      3) repo-local fallback: ./data/processed/scannet200/instance_gt/validation
    """
    env_path = os.environ.get("SSR3DLLM_SCANNET200_INSTANCE_GT_PATH", "").strip()
    if env_path:
        return env_path

    split = os.environ.get("SSR3DLLM_SCANNET200_INSTANCE_GT_SPLIT", "validation").strip() or "validation"
    try:
        from config import SCANNET_PROCESSED_ROOT  # type: ignore

        return str(Path(SCANNET_PROCESSED_ROOT) / "instance_gt" / split)
    except Exception:
        return "./data/processed/scannet200/instance_gt/validation"
...
class Evaluator():
    ...
    def average_two_score(iou50,iou25,print_score=True):
        if iou50 is None or iou25 is None or len(iou50) == 0 or len(iou25) == 0:
            if print_score:
                print("iou 50 n/a (empty)")
                print("iou 25 n/a (empty)")
    ...
    def evaluate_detection(self):
        # NOTE: `scene*.npz` stores full-res `pred_masks` which can be very large.
        # Loading all `.npz` into a dict can cause CPU OOM (process killed) on full eval.
        # Use a streaming evaluator that reads one scan at a time.
        all_detection_files = sorted(glob.glob(f"{self.directory_path}/*.npz"))
        if not all_detection_files:
            print("[WARN] detection eval skipped: no *.npz found under", self.directory_path)
            return

        gt_path = _resolve_scannet200_instance_gt_path()
        gt_dir = Path(gt_path)
        if not gt_dir.exists():
            print(f"[WARN] detection eval skipped: gt_path not found: {gt_path}")
            return

        pred_files = []
        missing = []
        for p in all_detection_files:
            scene_name = Path(p).stem
            if (gt_dir / f"{scene_name}.txt").is_file():
                pred_files.append(p)
            else:
                missing.append(scene_name)
        if missing:
            print(
                f"[WARN] detection eval: missing GT for {len(missing)} scenes under {gt_path} "
                f"(showing up to 5): {missing[:5]}"
            )
        if not pred_files:
            print("[WARN] detection eval skipped: no scenes with matching GT files.")
            return

        try:
            from benchmark.evaluate_semantic_instance import evaluate_from_npz_files
            ...
            print(f"[WARN] detection eval failed: {e}")
            return
    ...
    def load_data_and_eval(self):
        if test_m3drefer:
            try:
                ...
                if not pred_data:
                    print("[WARN] m3drefer official evaluator skipped: empty predictions (likely no m3dref samples in this run).")
                else:
            ...
            if filename.endswith('.json'):
                ...
                with open(file_path, 'r') as file:
                    ...
                    for line in data["prediction"]:
                        # Use JSON filename stem as the scene id (e.g. `scene0011_00.json` -> `scene0011_00`).
                        line["scene_id"] = Path(file_path).stem
        ...
        if self.statistics:
            ...
            if total_det <= 0:
                print("gt probability of det n/a (total_det=0)")
                print("pred probability of det n/a (total_det=0)")
                print("f1 n/a (total_det=0)")
            else:
